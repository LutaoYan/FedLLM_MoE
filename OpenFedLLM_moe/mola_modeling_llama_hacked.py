...
#ylt_modified
def load_balancing_loss_func(gate_logits: torch.Tensor, num_experts: torch.Tensor, top_k=2):
    r"""
    Computes auxiliary load balancing loss as in Switch Transformer - implemented in Pytorch.
    Also returns a list of dictionaries containing layer-wise statistics.
    """
    ...
    overall_loss = torch.tensor(0, dtype=torch.float32).to(compute_device)
    layer_stats = []  # List to store layer-wise statistics
    ...
    for layer_i in range(len(gate_logits)):
        ...
        # Convert tensor to numpy array for more efficient storage and then to list if needed
        expert_mask_1_numpy = expert_mask_1.detach().cpu().numpy().tolist()

        # Collect layer-wise statistics
        layer_stats.append({
            'Layer': int(layer_i),
            'T_tokens': int(torch.sum(expert_mask_1).item()),
            'T_distribution': expert_mask_1_numpy,
            'layer_loss': float(layer_loss.item())
        })

    return overall_loss, layer_stats
...
class LlamaForCausalLM_d(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # ylt_modified
        self.eval_records = []  # Initialize an empty list to store records
    ...
    def forward(
        ...
        layer_stats = None
        if output_router_logits:
            aux_loss, layer_stats = load_balancing_loss_func(
        ...
        if self.obalance:
            ...
            print('balance:', aux_loss.clone().detach().cpu())
            print('total loss:', loss.clone().detach().cpu())

        # ylt_modified
        record = {
            'obalance': self.obalance,
            'coef': self.router_aux_loss_coef,
            'aux_loss': float(aux_loss.item()) if aux_loss is not None else None,
            'loss': loss if loss is not None else None,
            'aux_stat': layer_stats,  # Add layer statistics to the record
        }
        self.eval_records.append(record)  # Append the record to the list
